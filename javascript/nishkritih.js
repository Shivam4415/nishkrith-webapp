(function (window, document, undefined) {
    ...
    N.Validators = {
        ...
        attachValidator: function (el) {
            ...
            var control = $("[" + identifyControlBy + "='" + data["mplValidateControl"] + "'", boundary.length ? boundary : null).first();
            ...
            var validateOnBlur = (data["mplValidationOnblur"] == undefined ? true : (data["mplValidationOnblur"] === false ? false : true));
            ...
            if (type != "") {
                ...
                var params = { control: control, validator: validator, message: message, element: element };
                var func = null;
                switch (type) {
                    case "required": {
                    ...
                    case "email": func = N.Validators.email; break;

                    case "password": func = N.Validators.password; break;

                    case "number": func = N.Validators.number; break;
                    ...
                    case "recaptcha": func = N.Validators.recaptcha; break;

                    case "state": {
                        func = N.Validators.state;
                        if (data["mplInfoControl"]) {
                            params["infoControl"] = $("[" + identifyControlBy + "='" + data["mplInfoControl"] + "'", boundary.length ? boundary : null).first();
                        }
                        break;
                    }
                }
                ...
                N.Validators.pageValidators.push({ group: group, func: func, params: params });
        ...
        password: function (params) {
            ...
            if (password.length > 0) {
                if (N.Global.regexPassword.test(password)) {
                    ...
                    control.removeClass("uk-form-success").addClass("uk-form-danger");
                    validator.text("Your password should have minimum of 8 characters with at least 1 letter and 1 number or special character.").removeAttr("hidden");
                    return false;
                }
            }
            else {
        ...
        state: function (params) {
            ...
            if (!(infoControl && infoControl.length))
                return true;

            //Deciding validation error message depends on country value
            var message = (infoControl.val() == "US") ? "Please enter your state." : "Please enter your province.";
            ...
            for (var i = 0; i < N.Validators.pageValidators.length; i++) {
                try {
                    ...
                    if (validator.group === group)
                        if (!validator.func(validator.params))
                            validated = false;
                }
            ...
            for (var i = 0; i < N.Validators.pageValidators.length; i++) {
                ...
                if (validator.group === group) {
                    if (validator.params.control.is('select')) {
                        validator.params.control.next().children().first().removeClass("uk-form-success").removeClass("uk-form-danger");
                    }
                    else {
                        validator.params.control.removeClass("uk-form-success").removeClass("uk-form-danger");
                    }
        ...
        removeValidate: function (group) {
            group = group || '';
            N.Validators.pageValidators = N.Validators.pageValidators.filter(function (el) { return el.group !== group });
        },
    }
